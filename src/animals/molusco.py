...
class Molusco(animal.Animal):
    def __init__(self):
        """ Initializes Molusco, inheriting Animal attributes and adding tipo_estrella_erizo. """
        super().__init__()
        self.tipo_estrella_erizo = str

    def set_additional_information_animal(self) -> None:
        """ Asks the user for weight, color and subtype (erizo or estrella) specific to a mollusc.\n
        return None """
    ...
    def send_data_to_file_txt(self) -> None:
        """ Writes the mollusc's name, weight, color and subtype to its txt file.\n
        return None """
